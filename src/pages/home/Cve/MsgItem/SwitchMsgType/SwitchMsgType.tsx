...
import { customMessageType, messageTypes } from "../../../../../constants/messageContentType";
...
import { seconds2HMS } from "../../../../../utils/common";
  ...
  const msgType = () => {
    switch (msg.contentType) {
      ...
      case messageTypes.LOCATIONMESSAGE:
        ...
        return (
          <div className={`chat_bg_msg_content_map ${!isSingle ? "nick_magin" : ""}`} onClick={()=>window.open(`https://gaode.com/regeo?lng=${postion.longitude}&lat=${postion.latitude}`)}>
            <Map protocol="https" center={postion} zoom={16} amapkey="dcdc861728801ee3410f67f6a487d3fa" >
      ...
      case messageTypes.CUSTOMMESSAGE:
        const data = msg.customElem.data;
        const map:CustomType = JSON.parse(data);
        let isMe = isSelf(msg.sendID);
        switch (map.customType) {
          case customMessageType.call:
          const {duration,state,type} =map.data as CallStateType;
          let content = '';

          // TODO 和 flutter一致 chatlogic.dart:1370
          switch (state) {
            case 'BE_HANGUP':
            case 'HANGUP':

              content=`${t('callDuration')} ${seconds2HMS(duration)}`;
              break;
            case 'CANCEL':
              content=isMe ? t('cancelled'): t('cancelledByCaller');
              break;
            case 'REJECT':
              content=isMe ? t('rejected'): t('rejectedByCaller');
              break;
            case 'BUSY':
              content=isMe ? t('busy'): t('busyByCalle');
              break;
          }
            return <div className={`chat_bg_msg_content_text ${!isSingle ? "nick_magin" : ""}` }>{content}</div>
        }
